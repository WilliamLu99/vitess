...
const getInitialSchemaVersions = "select id, pos, ddl, time_updated, schemax from %s.schema_version where from_unixtime(time_updated) < date_sub(now(), interval ? second) order by id asc"
...
func (h *historian) loadFromDB(ctx context.Context) error {
	...
	if h.lastID == 0 && h.schemaMaxAgeSeconds > 0 { // only at vttablet start
		tableData, err = conn.Exec(ctx, sqlparser.BuildParsedQuery(getInitialSchemaVersions, sidecardb.GetIdentifier(),
			h.schemaMaxAgeSeconds).Query, 10000, true)
	} else {
...
func (h *historian) purgeOldSchemas() {
	...
	for _, s := range h.schemas {
		if time.Now().Sub(time.Unix(s.timeUpdated, 0)) < maxAgeDuration {
