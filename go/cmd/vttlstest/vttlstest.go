...
func cmdCreateCRL(subFlags *flag.FlagSet, args []string) {
	subFlags.Parse(args)
	if subFlags.NArg() != 1 {
		log.Fatalf("CreateCRL command takes a single name as a parameter")
	}
...
func cmdRevokeCert(subFlags *flag.FlagSet, args []string) {
	ca := subFlags.String("ca", "ca", "ca cert name to use. Use 'ca' for the toplevel CA.")
	...
	name := subFlags.Arg(0)
	tlstest.RevokeCertAndRegenerateCRL(*root, *ca, name)
}
